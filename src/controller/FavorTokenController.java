...
public class FavorTokenController {
	...
	public void assignFavorTokens(int idGame, int difficulty, int idPlayer) {
		String query = ("SELECT * FROM gamefavortoken WHERE idplayer IS NULL AND idgame = " + idGame + " LIMIT "
				+ difficulty);
	...
	public void updateFavorToken(int idFavortoken, int idPlayer) {
		dbController.doUpdateQuery(
				"UPDATE gamefavortoken SET idplayer = " + idPlayer + " WHERE idfavortoken = " + idFavortoken);
	}
